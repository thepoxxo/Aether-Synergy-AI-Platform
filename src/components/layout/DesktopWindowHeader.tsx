...
export const DesktopWindowHeader: React.FC = () => {
  const { deviceMode, setDeviceMode, setIsSimulatorModalOpen } = useDeviceMode();
    ...
    <div className="hidden lg:flex items-center justify-between px-4 py-1.5 bg-cyber-950 border-b border-cyber-900 text-slate-400 font-mono text-[11px] select-none">
      ...
      <div className="flex items-center gap-1 bg-cyber-900/80 px-2 py-0.5 rounded-xl border border-cyber-800">
        ...
        <button
          ...
          className="px-2 py-0.5 rounded-lg text-[10px] flex items-center gap-1 bg-cyber-950 hover:bg-cyber-800 text-cyan-300 border border-cyan-500/40 transition-all"
          title="Abrir Simulador Interactivo de Smartphone (iPhone / Android)"
        >
          <Smartphone className="w-3 h-3 text-cyan-400" />
          <span>📱 Probar App Móvil</span>
        </button>
      </div>
